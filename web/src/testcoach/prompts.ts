...
export function buildSegmentPrompt(category: string, expectedLine?: string) {
  const lineGate = expectedLine
    ? `

[정해진 대사 확인]
이 연습에는 정해진 대사가 있습니다: "${expectedLine}"
- 배우가 영상에서 이 대사를 실제로 말했으면 spokeExpectedLine=true (억양·조사·사소한 말바꿈은 무방, 일부만 말해도 핵심 내용이 들리면 true).
- 전혀 다른 말을 하거나, 대사 없이 표정·동작만 있거나, 들리는 말이 없으면 spokeExpectedLine=false.`
    : `

[정해진 대사 확인]
정해진 대사가 없으므로 spokeExpectedLine=true로 둡니다.`;
...
- 망설여지면 detected=false. 억지로 자르지 않습니다.${lineGate}

JSON만 반환.`;
}

export function buildIntentPrompt(category: string, intentRaw: string) {
  return `배우가 받은 연기 상황(과 정해진 대사)을 분류합니다. 새 내용을 지어내지 마세요.

분류: ${category}
연기 상황: ${intentRaw}

작업:
1. intent_keywords — 위 내용에서 핵심 단어를 그대로 추출(최대 3개). 바꿔 쓰지 않는다.
2. intent_summary — 위 내용을 한두 줄(최대 80자)로 압축. 쓰인 단어를 그대로 살려 쓰고,
   없는 단어·해석·감정을 더하지 않는다(발췌·재배열만). 이미 짧으면 그대로 적는다.
3. expected_impressions — 위 내용이 직접 가리키는 인상만(최대 2개):
   분노, 슬픔, 평정, 두려움, 기대, 절망, 기쁨, 체념.
   내용에 근거가 있어야 한다. 장면 분위기로 짐작해 없는 인상을 더하지 않는다.
   확신 없으면 1개만, 그래도 없으면 빈 배열.
4. confidence — 상황이 또렷하면 "높음", 모호하면 "낮음".
