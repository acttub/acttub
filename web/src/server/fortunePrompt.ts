export type Fortune = {
  oneLiner: string;
  emotion: string;
  condition: { stars: number; comment: string };
  line: string;
  mission: string;
};

export type FortuneSeed = {
  birth: string;
  role: string;
  work: string;
  today: string;
};

export function buildFortunePrompt(seed: FortuneSeed): string {
  return [
    '너는 배우와 연기 지망생을 위한 "오늘의 연기 운세"를 봐주는 점술가다.',
    '아래 정보를 바탕으로 오늘 하루의 연기 운세를 만들어라.',
    '',
    `오늘 날짜: ${seed.today}`,
    `생일: ${seed.birth} (별자리/사주 느낌의 운세 시드로 활용)`,
    `배역: ${seed.role}`,
    `작품: ${seed.work}`,
    '',
    '톤: 가볍고 위트있게, MZ 운세 앱 느낌. 진지하게 가르치지 말 것.',
    '배역과 작품을 반드시 살려 개인화할 것. 명대사는 해당 작품/배역 맥락의 유명한 것을 쓰되, 애매하면 느낌을 살린 한 줄로.',
    '감정 키워드는 연기의 "감정표현"과 연결되게 한 단어~짧은 구로.',
    '미션은 30초 안에 할 수 있는 가벼운 연습 한 줄.',
    '',
    '반드시 아래 JSON 형식으로만 응답하라. 코드펜스나 다른 설명 없이 JSON 객체만 출력:',
    '{',
    '  "oneLiner": "오늘의 한 줄 (그날의 핵심 메시지)",',
    '  "emotion": "오늘의 감정 키워드 (짧게)",',
    '  "condition": { "stars": 1에서 5 사이 정수, "comment": "연기 컨디션 한 줄" },',
    '  "line": "오늘의 명대사 한 줄",',
    '  "mission": "오늘의 미션 한 줄"',
    '}',
  ].join('\n');
}

function extractJson(text: string): string {
  const fenced = text.match(/```(?:json)?\s*([\s\S]*?)```/i);
  const raw = fenced ? fenced[1] : text;
  const start = raw.indexOf('{');
  const end = raw.lastIndexOf('}');
  if (start === -1 || end === -1 || end < start) {
    throw new Error('운세 응답을 해석하지 못했습니다.');
  }
  return raw.slice(start, end + 1);
}

export function parseFortune(text: string): Fortune {
  const parsed = JSON.parse(extractJson(text)) as Record<string, unknown>;
  const condition = (parsed.condition ?? {}) as Record<string, unknown>;
  const starsNum = Number(condition.stars);
  const stars = Number.isFinite(starsNum) ? Math.min(5, Math.max(1, Math.round(starsNum))) : 3;
  const str = (value: unknown) => (typeof value === 'string' ? value.trim() : '');

  const fortune: Fortune = {
    oneLiner: str(parsed.oneLiner),
    emotion: str(parsed.emotion),
    condition: { stars, comment: str(condition.comment) },
    line: str(parsed.line),
    mission: str(parsed.mission),
  };

  if (!fortune.oneLiner || !fortune.emotion || !fortune.line || !fortune.mission) {
    throw new Error('운세 응답이 비어 있습니다.');
  }

  return fortune;
}
