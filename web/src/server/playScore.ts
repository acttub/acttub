...
// 추측작 vs 정답작의 속성 6칸을 비교해 워들식 그리드(hit/near/miss)와 꼬맨틀식 근접도%를 낸다.
...
export type StagedHint = { quote?: string; role?: string; synopsis?: string };
...
const CAST_ORDER: Record<string, number> = { '1인극': 0, '2인극': 1, 소규모: 2, 앙상블: 3 };
// 국가 권역 — 같은 권역이면 near. 영미권 / 대륙유럽 / 아시아.
const COUNTRY_REGION: Record<string, string> = {
  영국: 'anglo', 미국: 'anglo', 아일랜드: 'anglo',
  프랑스: 'euro', 독일: 'euro', 러시아: 'euro', 노르웨이: 'euro', 그리스: 'euro',
  한국: 'asia',
};
...
function orderState(order: Record<string, number>, a: string, b: string): AttrState {
  const da = order[a];
  const db = order[b];
...
export function scoreGuess(guess: PlayWork, answer: PlayWork): Score {
  const grid: GridCell[] = [
    ...
    { key: 'era', label: '시대', state: orderState(ERA_ORDER, guess.era, answer.era), value: guess.era },
    ...
    { key: 'cast', label: '인물', state: orderState(CAST_ORDER, guess.cast, answer.cast), value: guess.cast },
  ];
...
// 단계 힌트: 시도가 쌓일수록 명대사→배역→줄거리 순으로 공개.
export function stagedHint(work: PlayWork, attempt: number): StagedHint {
  const hint: StagedHint = {};
  if (attempt >= 2) hint.quote = work.hints.quote;
  if (attempt >= 3) hint.role = work.hints.role;
  if (attempt >= 4) hint.synopsis = work.hints.synopsis;
  return hint;
}
