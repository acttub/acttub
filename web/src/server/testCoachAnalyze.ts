...
import { PRESCRIPTION_LIBRARY, ROOT_DICTIONARY } from '../testcoach/dictionaries';
import { aggregate, assembleCard, mergeToAnchors, pickMajority } from '../testcoach/pipeline';
import {
...
type CoachSecondInput = {
  ...
  category: string;
  intent: string;
  // 정해진 대사(빈 문자열이면 대사 게이트 미적용 = 항상 통과).
  expectedLine: string;
  startTime: number;
  endTime: number;
  apiKey: string;
...
// 정해진 대사를 안/다르게 말함 — 분석 중단 sentinel(피드백 대신 재요청 신호).
const LINE_NOT_SPOKEN = '__LINE_NOT_SPOKEN__';
  ...
  try {
    ...
    try {
      const detected = parseStructured<SegmentResult>(
        await generate(buildSegmentPrompt(input.category, input.expectedLine), SEGMENT_SCHEMA, true),
      );
      // 정해진 대사를 안/다르게 말했으면 분석을 진행하지 않고 중단 → 재요청 안내.
      if (input.expectedLine && detected.spokeExpectedLine === false) {
        throw new Error(LINE_NOT_SPOKEN);
      }
      ...
      if (error instanceof Error && error.message === LINE_NOT_SPOKEN) throw error;
...
export async function handleTestCoachAnalyze(request: Request, options: CoachSecondOptions = {}): Promise<ApiResult> {
  ...
  try {
    ...
    if (contentType.includes('application/json')) {
      ...
      const expectedLine = jsonString(payload?.expectedLine);
    ...
    const expectedLine = textFromForm(formData.get('expectedLine'));
    ...
    // 정해진 대사를 하지 않음 — 분석 미수행, 재요청 신호(200, feedback 없음).
    if (error instanceof Error && error.message === LINE_NOT_SPOKEN) {
      return { status: 200, body: { spokeExpectedLine: false } };
    }
