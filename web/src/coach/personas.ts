...
// 공통 few-shot — 톤·형식 고정(프롬프트 주입 학습). 갭(aligned=false)과 강점(aligned=true)을
// 모두 보여 양쪽 형식을 잡는다. 우영이 실제 좋은 예시로 교체 가능.
const PERSONA_SHOT = `예시(형식만 참고, 톤을 모방하되 내용은 관찰에서 가져올 것):
...
{ "timecode": "0:31", "observed": "\\"왜 그랬어\\" 앞에서 0.8초 멈췄다가 낮은 음으로 내뱉음", "read": "분노를 터뜨리지 않고 눌러 담으려 하신 것 같다", "seen": "그 멈춤 덕에 참는 긴장이 또렷이 전해진다", "tip": "이 멈춤-누른 발성 조합을 다음 격앙 대사에도 한 번 더 써본다", "aligned": true }`;
