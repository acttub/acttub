...
import { COL, commentVoteId, type CommentDoc } from "@/lib/firebase/schema";
...
export async function POST(req: Request, { params }: Ctx) {
  ...
  const db = adminDb();
  const commentRef = db.collection(COL.comments).doc(id);
  const voteRef = db
    .collection(COL.commentVotes)
    .doc(commentVoteId(user.id, id));
  ...
  try {
    await db.runTransaction(async (tx) => {
      ...
      if (comment.deletedAt !== null) throw new Error("NOT_FOUND");
      ...
      if (delta !== 0) {
        tx.update(commentRef, { score: FieldValue.increment(delta) });
        newScore = comment.score + delta;
      } else {
